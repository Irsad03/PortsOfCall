...
import mapSrc from '../assets/minigame/stone-dodge/test1.jpg';
...
const SPAWN_HINT_X = 150;
const SPAWN_HINT_Y = 360;
const START_A      = 0;

function isGreenGoalPixel(r, g, b) {
    return g > 130 && g > r + 25 && g > b + 15;
}

function classifyPixel(r, g, b) {
    if (isGreenGoalPixel(r, g, b)) return 'goal';
...
function findGoalRect(offCtx, mapW, mapH) {
    const data = offCtx.getImageData(0, 0, mapW, mapH).data;
    let minX = Infinity, maxX = -Infinity, minY = Infinity, maxY = -Infinity;
    let found = false;
    for (let y = 0; y < mapH; y += 2) {
        for (let x = 0; x < mapW; x += 2) {
            const i = (y * mapW + x) * 4;
            if (isGreenGoalPixel(data[i], data[i + 1], data[i + 2])) {
                if (x < minX) minX = x;
                if (x > maxX) maxX = x;
                if (y < minY) minY = y;
                if (y > maxY) maxY = y;
...
function pointInRect(px, py, rect) {
    return rect && px >= rect.x1 && px <= rect.x2 && py >= rect.y1 && py <= rect.y2;
}
...
const GOAL_SAFE_MARGIN = 14;

function shipCollides(offCtx, mapW, mapH, cx, cy, angle, goalRect) {
    const cos = Math.cos(angle);
    const sin = Math.sin(angle);
    const hw = HIT_W / 2;
    const hh = HIT_H / 2;
    const safe = goalRect ? expandRect(goalRect, GOAL_SAFE_MARGIN) : null;
    for (let i = -2; i <= 2; i += 1) {
        for (let j = -1; j <= 1; j += 1) {
            const lx = (i / 2) * hw;
            const ly = j * hh;
            ...
            if (safe && pointInRect(wx, wy, safe)) continue;
            const kind = pixelKindAt(offCtx, mapW, mapH, wx, wy);
            if (kind === 'obstacle') return true;
        }
    }
    return false;
}

function isInGoal(cx, cy, angle, goalRect) {
    if (!goalRect) return false;
    const winRect = expandRect(goalRect, 4);
    if (pointInRect(cx, cy, winRect)) return true;
    const cos = Math.cos(angle);
    const sin = Math.sin(angle);
    const half = SW / 2;
    for (let f = -1; f <= 1.0001; f += 1 / 3) {
        const px = cx + cos * half * f;
        const py = cy + sin * half * f;
        if (pointInRect(px, py, winRect)) return true;
    }
    return false;
}
...
export default function MinigameView({ onExit }) {
    ...
    useEffect(() => {
        ...
        mapImg.onload = () => {
            ...
            s.goalRect = findGoalRect(offCtx, s.mapW, s.mapH);
        ...
        mapImg.onerror = () => console.error('MinigameView: map image failed to load');
        mapImg.src = mapSrc;
        ...
        function loop(now) {
            ...
            if (turn !== 0) {
                ...
                if (!shipCollides(s.offCtx, s.mapW, s.mapH, s.x, s.y, newAngle, s.goalRect)) {
                    s.angle = newAngle;
                }
            }
            ...
            if (s.started && isInGoal(nx, ny, s.angle, s.goalRect)) {
                s.x = nx; s.y = ny;
                s.speed = 0;
                render();
                finish('success');
            ...
            if (s.started && Math.abs(s.speed) > 0.5) {
                ...
                const collided = oob || shipCollides(
                    s.offCtx, s.mapW, s.mapH, nx, ny, s.angle, s.goalRect,
                );
                if (collided) {
                    s.crashFlash = 1;
                    render();
        ...
        function render() {
            ...
            if (s.goalRect) {
                ...
                ctx.strokeStyle = `rgba(120, 255, 150, ${0.55 + 0.25 * Math.sin(t)})`;
                ctx.lineWidth = 2;
            ...
            ctx.textAlign = 'center';
            ctx.fillText(
                '⚓  Navigate through the obstacles to the green zone',
                CW / 2, 36,
            );
                ...
                <div style={{
                    ...
                    <FocusTrap style={{
                        ...
                        <h2 style={{
                            ...
                            {outcome === 'success' ? 'Made it!' : 'Crash!'}
                        </h2>
                        <p style={{ color: 'var(--text-secondary)', fontSize: '15px', margin: '0 0 18px' }}>
                            {outcome === 'success'
                                ? 'You steered the ship safely into the green zone.'
                                : 'Your ship hit an obstacle.'}
                        </p>
                        <button
